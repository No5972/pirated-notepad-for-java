...
public class Notepad extends JFrame {
    ...
    public Notepad() {
        ...
        replaceMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_R, InputEvent.CTRL_DOWN_MASK)); // TODO 原有的快捷键 Ctrl+H 和JTextArea 的退格快捷键冲突，所以用 Ctrl+R 代替。使用 Ctrl+H 需要重写 JTextArea 类
        ...
        toggleWrapMenuItem.addActionListener(e -> toggleWrap());
        ...
        gbcStatusBar.gridx = 1; gbcStatusBar.gridy = 0; gbcStatusBar.weightx = 0.2;
        gbcStatusBar.anchor = GridBagConstraints.WEST;
        statusBarContainer.add(caretPositionLabel, gbcStatusBar);
        gbcStatusBar.gridx = 2; gbcStatusBar.gridy = 0; gbcStatusBar.weightx = 0.1;
        gbcStatusBar.anchor = GridBagConstraints.WEST;
        statusBarContainer.add(encodingLabel, gbcStatusBar);
        gbcStatusBar.gridx = 3; gbcStatusBar.gridy = 0; gbcStatusBar.weightx = 0.1;
        ...
        statusBarContainer.setPreferredSize(new Dimension(BigDecimal.valueOf(statusBarContainer.getPreferredSize().getWidth()).intValue(), 20));
        ...
        if (result == JFileChooser.APPROVE_OPTION) {
            ...
            try  {
                ...
                if (encoding.equals("OTHER")) {
                    statusBar.setText("警告：尝试打开不支持的编码的文件");
                }
            } catch (IOException e) {
                JOptionPane.showMessageDialog(this, "打开文件失败: " + e.getMessage(), "错误", JOptionPane.ERROR_MESSAGE);
            }
    ...
    private boolean saveAsFile(boolean isSaveAs) {
        ...
        if (result == JFileChooser.APPROVE_OPTION) {
            File file = fileChooser.getSelectedFile();
            String saveNewFile = file.getAbsolutePath();
    ...
    private void toggleWrap() {
        ...
        // this.caretPositionLabel.setVisible(!textArea.getLineWrap());
    ...
    public String detectEncoding(String filePath) {
        File fileToDetect = new File(filePath);
        if (fileToDetect.length() <= 0) {
            return "UTF-8"; // 空文件默认使用UTF-8
        } else {
            BytesEncodingDetect sinodetector = new BytesEncodingDetect();
            return Encoding.nicename[sinodetector.detectEncoding(new File(filePath))];
        }
    }
    ...
    class FindDialog extends JDialog {
        ...
        public FindDialog(Notepad parent, JTextArea textArea) {
        ...
        public FindDialog(Notepad parent, JTextArea textArea, String findText) {
            super(parent, "查找", false);
            this.parent = parent;
            this.textArea = textArea;
        ...
        private void initializeUI() {
            ...
            JPanel checkboxesPanel = new JPanel(new GridLayout(2, 1));
            checkboxesPanel.add(matchCaseCheckBox);
            checkboxesPanel.add(loopCheckBox);
            caseSensitiveAndDirectionPanel.setBorder(BorderFactory.createEmptyBorder(0, 0, 0, 0));
            caseSensitiveAndDirectionPanel.add(checkboxesPanel);
    ...
    class ReplaceDialog extends JDialog {
        ...
        public ReplaceDialog(Notepad parent, JTextArea textArea) {
        ...
        public ReplaceDialog(Notepad parent, JTextArea textArea, String findText) {
            super(parent, "替换", false);
            this.parent = parent;
            this.textArea = textArea;
            initializeUI(findText);
...
//            optionPanel.add(loopCheckBox);
        ...
        private void replaceAll() {
            ...
            if (count > 0) {
                ...
                parent.statusBar.setText("已替换 " + count + " 处匹配项");
            } else {
                JOptionPane.showMessageDialog(this, "找不到 \"" + textToFind + "\"。", "提示", JOptionPane.INFORMATION_MESSAGE);
            }
